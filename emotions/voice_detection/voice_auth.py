...
from emotions.voice_detection.feature_extraction import get_embedding, get_embeddings_from_list_file
from emotions.voice_detection.preprocess import get_fft_spectrum
import emotions.voice_detection.parameters as p
...
def recognize(file):
    ...
    if min(list(distances.values()))<p.THRESHOLD:
        name_given = min(distances, key=distances.get) 
